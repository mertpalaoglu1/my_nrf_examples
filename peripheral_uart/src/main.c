...
#define RESET_BUTTON_NODE DT_ALIAS(sw3)
...
{
    LOG_INF("Reset button pressed. Resetting...");
    NVIC_SystemReset();
}
...
static void configure_reset_button(void)
{
    ...
    if (!device_is_ready(reset_button.port)) {
        LOG_ERR("Reset button GPIO device not ready");
        return;
    }

    ret = gpio_pin_configure_dt(&reset_button, GPIO_INPUT);
    if (ret != 0) {
        LOG_ERR("Failed to configure reset button pin");
        return;
    }
    ...
    gpio_init_callback(&reset_button_cb, reset_button_pressed, BIT(reset_button.pin));
    gpio_add_callback(reset_button.port, &reset_button_cb);
}
			  ...
			  uint16_t len)
{
	...
	// === LED2'yi Yak ===
    dk_set_led_on(DK_LED3);         // LED2 yanar
    k_sleep(K_MSEC(100));           // 100 ms bekle
    dk_set_led_off(DK_LED3);        // LED2 söner
...
int main(void)
{
	...
	configure_gpio();
	configure_reset_button();  // buraya ekle
	err = uart_init();
	if (err) {
		error();
	}
